...
- **PLU + extraction IA** (US-2.1) : zonage via API Carto GpU + règlement parsé par IA (`ANTHROPIC_API_KEY` côté worker), citation de l'article source et confiance. **Prochaine slice enrichissement, la plus délicate** (clé Anthropic requise dans le `.env` Arcane).
