...
- **Services de proximité** (US-2.5) : OSM (écoles, commerces, transports), type `SERVICES` sur le socle existant. Prochaine slice enrichissement.
...
- **PLU + extraction IA** (US-2.1) : zonage via API Carto GpU + règlement parsé par IA (`ANTHROPIC_API_KEY` côté worker), citation de l'article source et confiance. Le plus délicat.
